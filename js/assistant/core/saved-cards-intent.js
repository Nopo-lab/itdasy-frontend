...
(function () {
  ...
  var OPEN_VERB = /(열어|보여|가자|가줘|가서|이동|들어가|띄워|들어갈|시작)/;
  ...
  // bare "X 카드 보여줘"가 고객/예약/매출 카드 같은 다른 도메인으로 새지 않게(저장단서/명시 없을 때만 차단).
  var NOT_SAVED_RE = /(고객|손님|명단|연락처|전화번호|예약|매출|통계|차트|일정|회원)/;
  ...
  function classify(q) {
    ...
    // 2) 순수 생성 제외 — 저장단서/다시/조회·편집 동사가 하나도 없을 때만 양보.
    if (CREATE_RE.test(t) && !SAVED_CUE.test(t) && !/다시/.test(t) && !hasShow && !hasEdit) return null;
    // 3) 명시 저장카드 → 무조건 조회/편집.
    var explicit = EXPLICIT_RE.test(t);
    // 4) 대상 + (조회|편집) 동사  또는  저장단서 + 대상 + 조회 동사.
    //    bare 대상+동사 경로는 고객/예약/매출 등 타도메인 단어가 섞이면(저장단서 없을 때) 양보.
    var bare = TARGET_RE.test(t) && (hasShow || hasEdit) && !(NOT_SAVED_RE.test(t) && !SAVED_CUE.test(t));
    var matched = explicit
      || bare
      || (SAVED_CUE.test(t) && TARGET_RE.test(t) && hasShow);
