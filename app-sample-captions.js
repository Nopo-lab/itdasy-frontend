  ...
  function openSamplePopup() {
    ...
    if (!popup) {
      ...
      popup.innerHTML = `
        <div style="width:100%; max-width:480px; background:#fff; border-radius:24px 24px 0 0; padding:24px 20px calc(32px + env(safe-area-inset-bottom)); max-height:92vh; overflow-y:auto;">
          ...
          <div style="font-size:13px; color:#666; margin-bottom:14px;">인스타 연동 전에도 <b>이런 느낌</b>으로 캡션이 나와요. 연동하시면 <b>사장님 말투 그대로</b> 맞춤 생성됩니다.</div>
    ...
    list.innerHTML = samples.map((s, i) => `
      <div style="background:#fafafa; border-radius:14px; padding:14px; margin-bottom:10px;">
        <div style="font-size:13px; font-weight:800; color:var(--accent, #f18091); margin-bottom:6px;">${s.tone}</div>
        <div style="font-size:13px; color:#333; line-height:1.7; white-space:pre-wrap; margin-bottom:8px;">${s.caption.replace(/</g,'&lt;')}</div>
        <div style="font-size:13px; color:#888;">${s.hashtags.split(',').map(h => '#'+h.trim()).join(' ')}</div>
        <button data-sample-idx="${i}" class="_sampleCopyBtn" style="margin-top:10px; padding:8px 14px; border-radius:8px; border:1px solid var(--accent, #f18091); background:#fff; color:var(--accent, #f18091); font-size:13px; font-weight:700; cursor:pointer; min-height:36px;">📋 이 톤으로 복사</button>
      </div>
    `).join('');
