...
(function () {
  ...
  function _bgCard(bg, on) {
    ...
    return `<button type="button" data-pe-bg-id="${_esc(bg.id)}"
      ...
      ${preview}<div style="position:absolute;left:0;right:0;bottom:0;padding:3px 6px;background:rgba(0,0,0,0.55);color:#fff;font-size:11px;font-weight:700;text-align:center;">${_esc(bg.name)}</div>
    </button>`;
  }
